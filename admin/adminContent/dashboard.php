...
if ($conn) {
  ...
  $q = "SELECT COUNT(*) AS total FROM userinfo WHERE remarks IS NOT NULL AND remarks != ''";
...
<head>
  ...
  <style>
    ...
    .residency-card.foreign {
      border-color: #0A5C54;
    }

    .residency-card.bonified:hover {
      box-shadow: 0 0 15px rgba(45, 212, 201, 0.4);
    }

    .residency-card.migrant:hover {
      box-shadow: 0 0 15px rgba(25, 175, 165, 0.4);
    }

    .residency-card.transient:hover {
      box-shadow: 0 0 15px rgba(18, 125, 117, 0.4);
    }

    .residency-card.foreign:hover {
      box-shadow: 0 0 15px rgba(10, 92, 84, 0.4);
    }
              ...
              <div class="population-section text-white p-3 p-md-4 rounded-3 shadow-sm h-100">
                ...
                <div class="d-flex flex-column gap-2">
                  <div class="d-flex justify-content-between align-items-center p-2 bg-white bg-opacity-10 rounded-2">
                    <span class="small">Male</span><span class="small fw-bold"><?= number_format($maleCount) ?>
                      (<?= $malePercent ?>%)</span>
                  </div>
                  <div class="d-flex justify-content-between align-items-center p-2 bg-white bg-opacity-10 rounded-2">
                    <span class="small">Female</span><span class="small fw-bold"><?= number_format($femaleCount) ?>
                      (<?= $femalePercent ?>%)</span>
                  </div>
                  <div class="d-flex justify-content-between align-items-center p-2 bg-white bg-opacity-10 rounded-2">
                    <span class="small">Kids (0–12)</span><span class="small fw-bold"><?= number_format($kids) ?></span>
                  </div>
                  <div class="d-flex justify-content-between align-items-center p-2 bg-white bg-opacity-10 rounded-2">
                    <span class="small">Teenagers (13–17)</span><span
                      class="small fw-bold"><?= number_format($teens) ?></span>
                  </div>
                  <div class="d-flex justify-content-between align-items-center p-2 bg-white bg-opacity-10 rounded-2">
                    <span class="small">Adults (18–59)</span><span
                      class="small fw-bold"><?= number_format($adults) ?></span>
                  </div>
                  <div class="d-flex justify-content-between align-items-center p-2 bg-white bg-opacity-10 rounded-2">
                    <span class="small">Senior Citizens (60+)</span><span
                      class="small fw-bold"><?= number_format($seniors) ?></span>
                  </div>
                </div>
              </div>
            </div>
